...
def check_hate_speech(message):
    ...
    scores = [toxicity_score, severe_toxicity_score, identity_attack_score, insult_score, threat_score]
    ...
    if any(score > 0.8 for score in scores):  # if one score is really high, flag it
        return True
    if any(score > 0.3 for score in hate_speech_scores):  # if specific hate speech indicators are high, flag it
        return True
    ...
    # weights = [0.1, 0.25, 0.25, 0.1, 0.25]
